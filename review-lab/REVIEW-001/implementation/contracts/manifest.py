"""Manifest contract v1.0.0 — strict, fail-closed validation (ADR-CL-004).

Design: unknown field, unknown capability, unknown enum value, missing
required field, or quota above the signed node-policy ceiling all REJECT.
The reason is always logged by the caller; nothing is silently ignored.

The JSON Schema in manifest-v1.schema.json documents the same structure for
external tooling; this loader is the normative implementation for the slice.
"""
...
# Capability vocabulary v1. Note: NO actuation capabilities by design
# (safety gate — security-compliance-architecture §5).
...
class Manifest:
    ...
    quotas: Quotas
    fs_readwrite: tuple[str, ...]
    channels: tuple[ChannelDecl, ...] = field(default_factory=tuple)
...
def _require_exact_fields(obj: Mapping[str, Any], allowed: set[str], where: str) -> None:
...
def _contract_major(spec: str) -> int:
    # accepted forms: "1.x" or exact "1.0.0"-style; major is the leading int
...
def parse_manifest(doc: Mapping[str, Any]) -> Manifest:
    ...
    if unknown_caps:
        raise ManifestRejected(
            f"unknown capability {sorted(unknown_caps)} (fail closed)")
...
def check_against_policy(
    ...
    """Policy validation against the operator-signed node policy. Fail closed."""
